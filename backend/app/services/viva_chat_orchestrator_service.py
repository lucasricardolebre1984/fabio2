...
class VivaChatOrchestratorService:
    async def handle_chat_with_viva(self, request: Any, current_user: Any, db: Any):
        ...
        try:
            ...
            domain_query_response = await viva_domain_query_router_service.handle_domain_query(
                db=db,
                user_id=current_user.id,
                message=request.mensagem,
                contexto_efetivo=contexto_efetivo,
            )
            if domain_query_response:
                return await finalize(resposta=domain_query_response)
